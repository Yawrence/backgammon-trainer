class BackgammonBoard {
    ...
    getCheckerPosition(point, stackIndex) {
        ...
        const y = center.top
            ? 122 + stackIndex * this.geometry.checkerGap
            : 718 - stackIndex * this.geometry.checkerGap;

        return { x: center.x, y };
    }

    drawChecker(x, y, player, parent = this.svg) {
        const dark = player === "Oli" || player === "dark";

        const group = this.el("g", {}, parent);
    ...
    drawStack(point, player, count) {
        for (let i = 0; i < count; i++) {
            const pos = this.getCheckerPosition(point, i);
            this.drawChecker(pos.x, pos.y, player);
        }
    }
    ...
    animateMove(startPosition, endPosition, player, from, to, callback) {
        const movingStackCount =
            startPosition[from] && startPosition[from][player]
                ? startPosition[from][player]
                : 1;

        const fromPos =
            this.getCheckerPosition(from, movingStackCount - 1);

        const endStackCount =
            endPosition[to] && endPosition[to][player]
                ? endPosition[to][player]
                : 1;

        const toPos =
            this.getCheckerPosition(to, endStackCount - 1);

        const intermediate =
            JSON.parse(JSON.stringify(startPosition));

        if (intermediate[from] && intermediate[from][player]) {
            intermediate[from][player]--;

            if (intermediate[from][player] <= 0) {
        ...
        const checker =
            this.drawChecker(fromPos.x, fromPos.y, player);
